...
export default function ChatbotList() {
  ...
  const handleCopyId = async (id) => {
    try {
      ...
      window.prompt('Copy this chatbot ID:', id)
    }
  }
          ...
          {chatbots.map((chatbot) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <code className="text-[11px] font-mono text-gray-500 dark:text-gray-500 truncate" title={chatbot.id}>
                      {chatbot.id}
                    </code>
                    <button
                      type="button"
                      onClick={() => handleCopyId(chatbot.id)}
                      className="flex-shrink-0 p-1 -m-1 text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors"
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-gray-100 dark:border-dark-border">
                ...
                {chatbot.config?.wizardAnswers && (
                  <button
                    onClick={() => navigate(`/dashboard/agent-builder/chat/${chatbot.id}`)}
                    className="flex items-center justify-center gap-1.5 px-3 py-2 text-sm font-medium text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 rounded-lg hover:bg-primary-100 dark:hover:bg-primary-900/30 transition-colors"
