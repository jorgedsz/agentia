  ...
  try {
    ...
    if (decoded.isTeamMember && decoded.teamMemberId) {
      const teamMember = await req.prisma.teamMember.findUnique({
        ...
        include: {
          account: {
            select: { id: true, email: true, name: true, role: true, agencyId: true, outboundRate: true, inboundRate: true, voiceAgentsEnabled: true, chatbotsEnabled: true, crmEnabled: true, agentGeneratorEnabled: true, callsPaused: true, messagesPaused: true }
          }
        }
      });
